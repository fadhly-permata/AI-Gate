...
Usage: /run-impl [mode]
  fresh    = mulai dari awal (B0.1). Catat di documents/pm/status.md bahwa sesi fresh;
...
1. Load progress:
   ...
   - Baca `documents/pm/status.md` untuk penanda "task aktif" (jika ada).
2. Mode:
   - `fresh`: set penanda task aktif = B0.1 di documents/pm/status.md.
...
4. Eksekusi task aktif sesuai R9 (TANPA konfirmasi; ambiguitas -> default + log
   di documents/pm/status.md + memory-bank). Pakai sub-agent sesuai owner
...
5. **COMMIT TIAP SUBTASK (R19)** — begitu satu subtask selesai (receipt sub-agent
   ...
   JANGAN tandai [x].
6. Setelah task selesai: tandai `[x]` di BACKLOG.md, update documents/pm/status.md
...
- Implementasi jalan berurutan dari Backlog; progres tersimpan di BACKLOG.md +
  documents/pm/status.md DAN di git (checkpoint awal task + commit tiap subtask, R19)
