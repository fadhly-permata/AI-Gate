...
- 2026-09-09: **CLI Tools cross-platform compat harness + cli_compat→JSON refactor — DONE (branch `setup/cli-tools`).** Refactor `src/backend/cli_compat.py` jadi thin-loader atas `src/backend/cli_compat.json` (API `CLI_COMPAT`/`current_platform()`/`compat_for()` dipertahankan; `current_platform()` di-harden → balik `termux` di konteks terisolasi). Harness `scripts/cli-tools/compat-test.sh` memprobe 24 tool di platform saat ini (10 global = no run, 14 probe; gemini/codex→`not_wired`) lalu tulis kolom platform ke JSON (atomic, preserve platform lain). Doc self-contained `documents/pm/cli-tools-cross-platform-testing.md`. Verifikasi: `bash -n`+`py_compile` clean, `--dry-run` tampil 24 tool, write-path teruji. `--apply` sungguhan utk platform target (belum dijalankan di session ini — akan install nyata).
