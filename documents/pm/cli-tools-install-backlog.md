...
> **B1 openhands = [BUGFIX VERSION-GUARD 2026-09-09]** commit `313a2c2`: version-guard pre-install (have_cmd uv → lanjut; elif python3 major.minor !=3.12 → ERROR + exit 1 tanpa install; parse-gagal → WARN). bash -n clean; simulasi (uv absen): 3.12 lanjut, sisanya exit 1. Wiring launch utuh.
...
> **B5 gpt-researcher = [VERSION-GUARD N/A — 2026-09-09]** diverifikasi script TIDAK menjalankan pip install / uv tool install (cuma pesan + exit 0, NOT_A_CLI) → version-guard TIDAK ditambahkan, TIDAK diubah.
...
> **B6 crewai = [VERSION-GUARD N/A — 2026-09-09]** diverifikasi script TIDAK menjalankan pip install / uv tool install (cuma pesan + exit 0, NOT_A_CLI) → version-guard TIDAK ditambahkan, TIDAK diubah.
...
| C5 | gptme | gptme | `pip install gptme` | python (pip) | verified | done |
| C6 | aichat | aichat | `pkg install aichat` (override) / `cargo install aichat` | pkg (atau rust/cargo) | verified | done |
...
> **C5 gptme = done (2026-09-09):** install idempoten `pip install gptme` (fakta `cli_presets.py:104` = `pip install gptme`); launch **OpenAI-compatible** — gptme forward ke aigate `/v1/chat/completions` (wiring persis mirip `_gptme_builder` di `cli_tools_router.py:595-612`: set env `OPENAI_BASE_URL` = gateway base [PENTING: gptme membaca `OPENAI_BASE_URL`, BUKAN `OPENAI_API_BASE`] + `OPENAI_API_KEY` + flag `-m local/<model>`). Bukti `cli_presets.py:223` = `gptme` = `LAUNCH_VERIFIED` (OpenAI-compatible). PyPI `gptme` 0.33.0 butuh Python `>=3.10,<3.15`. **Catatan Termux (known-broken, bukan blocker):** di Termux/aarch64 + Python 3.14, `pip install gptme` diprediksi gagal build `jiter` (tidak ada wheel Android) → script handle hint + `exit 1`. `bash -n` clean; mode exec. Commit `16d36f9`.

> **C6 aichat = done (2026-09-09):** install via `TERMUX_INSTALL` → `pkg install aichat` (fakta `cli_presets.py:242` = `TERMUX_INSTALL["aichat"]="pkg install aichat"`, "verified 0.30.0 runs" di Termux) dengan fallback `cargo install aichat` (fakta `cli_presets.py:105` = `cargo install aichat`); launch **OpenAI-compatible** — aichat forward ke aigate `/v1/chat/completions` (wiring persis mirip `_aichat_builder` di `cli_tools_router.py:469-511`: generate `aichat-aigate.yaml` client `aigate` openai-compatible {api_base=gateway, api_key}, set env `AICHAT_CONFIG_FILE`, model `aigate:<raw>`). Bukti `cli_presets.py:226` = `aichat` = `LAUNCH_VERIFIED` (OpenAI-compatible). crates.io `aichat` 0.30.0 (Rust). **Catatan Termux (terbukti WORKING, bukan blocker):** `pkg install aichat` di Termux terbukti jalan (usable di Termux). `bash -n` clean; mode exec. Commit `1c4e592`.

> **GRUP C SELESAI (6/6):** seluruh 6 tool Grup C (`chat_shell`) selesai — C1 llm, C2 sgpt, C3 mods, C4 oterm, C5 gptme, C6 aichat. **ALL 24 DONE** (A1–A12, B1–B6, C1–C6) — seluruh 24 tool cli-tools SELESAI (script install/launch + wiring/NO_INSTALL/NOT_A_CLI sesuai preset).
