...
**Self-Heal (menu CLI-Tool):** lihat FSD §2.8. Backend helper `selfheal.run()`: (1) cek agentic CLI terinstall (`which` di Grup A); bila nihil → signal popup "Self-Heal tidak bisa jalan: tidak ada agentic CLI terinstall"; (2) `git init` bila belum ada repo, buat branch `aigate/self-heal-<ts>`; (3) query `LogEntry` severity warning|error; (4) jalankan agentic CLI DI DALAM sesi PTY terminal key `self-heal` (shell; CLI diketik sebagai command line, prompt dari file temp anti-injection, selesai ditandai file `.done` + poll) — UI otomatis membuka & memfokuskan tab tersebut sehingga progress fix terlihat live; (5) loop fix→test sampai tak ada warning/error (atau max iterasi). Run ASYNC: `POST /api/self-heal/run` → `{"ok":true,"started":true,"tab":"self-heal"}` (409 bila sudah jalan); `GET /api/self-heal/status` → `{"running","last"}` untuk polling UI.    LogWindow memantau progress. Setelah suatu issue selesai dikerjakan, baris
