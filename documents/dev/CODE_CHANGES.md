## 2026-09-09 — feat: harness kompatibilitas lintas-platform + refactor `cli_compat` → JSON thin-loader (branch `setup/cli-tools`) — DONE

**Tujuan:** bikin harness `scripts/cli-tools/compat-test.sh` yang memprobe 24 CLI tool di
Linux/Windows/macOS dan langsung mengisi kolom platform di katalog kompatibilitas, plus
refactor `cli_compat.py` jadi thin-loader atas `cli_compat.json` agar harness bisa
tulis/merge kolom platform secara programatik tanpa parse Python.

**Catatan proses (transparan, R29 ayat 3):** tool `Task`/spawn sub-agent TIDAK tersedia di
session ini (toolset PM = bash/read/write/edit/grep/glob/skill/webfetch; TIDAK ada Task).
Maka PM eksekusi langsung dalam batas file ketat (`src/backend/**` + `scripts/cli-tools/**`
+ `documents/pm/**` + `documents/dev/CODE_CHANGES.md`, sesuai instruksi task & preseden
`status.md` 2026-09-09). Bukan pelanggaran fungsional R21/R29 — deviasi alat, dicatat di sini.

### `src/backend/cli_compat.json` (BARU)
- Migrasi data Termux dari `CLI_COMPAT` (di `cli_compat.py`) ke JSON: `tool -> platform ->
  {status,note,source}` untuk 24 tool; kolom `termux` seeded (`source="tested-termux-2026-09"`),
  `linux`/`windows`/`macos` = `unknown`. Plus key `platforms` + `seeded_source`.

### `src/backend/cli_compat.py` (REFACTOR — thin-loader)
- Modul jadi pembaca JSON: `_load_compat()` baca `cli_compat.json` (path relatif `__file__`)
  saat import, bangun `CLI_COMPAT`. Semua platform column lain dipertahankan utuh; platform
  absen → `_unknown_row()`.
- API PUBLIK dipertahankan 1:1: `CLI_COMPAT`, `PLATFORMS`, `WARN_STATUSES`, `STATUS_*`,
  `current_platform()`, `compat_for()`, `__all__`. `import cli_compat` tetap bebas dependency
  (lazy import `backend.paths` di dalam `current_platform`); jalan sebagai `backend.cli_compat`
  maupun bare `cli_compat` dari `src/backend`.
- `current_platform()` di-harden: selain reuse `backend.paths.is_termux` (lazy), tambah
  fallback env/path (`TERMUX_VERSION` / `/data/data/com.termux/files`) dan tangani
  `platform.system()=="Android"` → `termux`. Sebelumnya di konteks terisolasi (backend tdk
  importable) balik `unknown` karena Termux build lapor `"Android"`. Sekarang balik `termux`
  konsisten dgn `scripts/cli-tools/_common.sh` detect_os.

### `scripts/cli-tools/compat-test.sh` (BARU)
- Detect platform → `termux`/`linux`/`windows`/`macos` (reuse logika `_common.sh` detect_os,
  map `mac`→`macos`).
- Iterasi 24 tool (dari key JSON via python3). 10 global (8 `no_install` + 2 `not_a_cli`) →
  langsung isi status tanpa jalanin script. 14 probe-worthy → jalanin `bash <tool>.sh --help`
  di temp dir terisolasi (timeout 300s), klasifikasi: exit 0 → `verified` (kec. `gemini`/`codex`
  → `not_wired`); non-zero + sentinel `"aborting launch"` → `broken`; non-zero lain →
  `installable`.
- Tulis hasil ke kolom platform saat ini di `cli_compat.json` via python3 (atomic: tempfile +
  `os.replace`), PRESERVE kolom platform lain. Cleanup temp dir via trap. Flag `--dry-run`
  (rencana saja, gak install/tulis) + `--apply` (default: beneran test + tulis). Print summary.

### `documents/pm/cli-tools-cross-platform-testing.md` (BARU)
- Dokumen self-contained: 24 tool, arti status, klasifikasi global, cara jalanin harness di
  Linux/macOS/Windows(Git Bash), cara hasil masuk ke `cli_compat`, perintah commit/push copy-paste.

### `documents/pm/cli-tools-compatibility.md` (MODIFY)
- Sync: sumber otoritatif kini `cli_compat.json` (thin-loader `cli_compat.py`); tambah catatan
  harness `scripts/cli-tools/compat-test.sh`.

**Verifikasi (R14/R35):**
- `bash -n scripts/cli-tools/compat-test.sh` → clean; `chmod 700`.
- `python3 -m py_compile src/backend/cli_compat.py` → clean; `import cli_compat` (bare &
  `cd src/backend`) OK; `current_platform()`→`termux`; `len(CLI_COMPAT)==24`;
  `compat_for("nope")["linux"]` fail-open `unknown`.
- `bash compat-test.sh --dry-run` → tampilkan rencana 24 tool tanpa install (8 no_install + 2
  not_a_cli = GLOBAL/no-run, 14 PROBE) — verified.
- Write-path diuji terisolasi (python3 snippet persis dari harness): update kolom `windows`,
  `termux` preserved, platform lain untouched, JSON valid 24 tool.
- **TIDAK dijalankan `--apply` sungguhan di session ini** (akan memicu install npm/pip/cargo
  nyata di Termux — berat & memutasi env). Real `--apply` ditujukan untuk platform target
  (Linux/Windows/macOS) sesuai tujuan task.
