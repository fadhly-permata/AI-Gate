...
### 2026-09-13 — FEAT bahasa Hindi (`hi`) (fe-dev; audit+verifikasi mandiri+commit PM) — commit `808d04e`, branch `feat/i18n-hindi`
**Asal:** user: "butuh bahasa Hindi di dalam aplikasi". Perintah (D2) → PM kerjakan tanpa tanya; default yang dipakai: kode `hi`, endonim `हिन्दी`, bendera `🇮🇳` (Hindi standar Devanagari, bukan romanisasi/Hinglish). Handover `documents/pm/handovers/handover-20260913-bahasa-hindi.md`.
**Arsitektur i18n (fakta):** 1 berkas kamus per bahasa `static/i18n/<code>.js` (self-register `window.I18N.<code>`); registry tunggal `window.LANGS` di `static/i18n.js:28-36`; paritas kunci dijaga `tests/i18n.test.js:88-101` (glob `static/i18n/*.js` — bahasa baru wajib kunci persis = `en`). Nama bahasa = endonim (`lang.<code>`) ada di SEMUA kamus.
**Per-berkas:**
- `src/frontend/static/i18n/hi.js` (BARU, +454): kamus Hindi, **445 kunci** (444 kunci en + `lang.hi`), terjemah Devanagari. `app.title`="aigate", `nav.repo`="aigate Repo" (merek tak diterjemah; dijaga `tests/i18n.test.js:217-220`), placeholder `{n}`/`{cli}`/`%s` dipertahankan.
- `src/frontend/static/i18n.js:36`: registry +1 baris `{ code: "hi", flag: "🇮🇳", nameKey: "lang.hi" }` (di akhir list).
- `src/frontend/static/i18n/{en,id,ru,nl,ja,zh,zh-tw}.js` (masing +1): kunci `"lang.hi": "हिन्दी"` (endonim konvensi berjalan) → semua 8 kamus jadi 445 kunci, paritas lolos.
- `src/frontend/static/index.html`: cache-buster `20260923`→`20260924` (`V` :20, `i18n.js?v=` :1436, `app.js?v=` :1446) → invarian `tests/i18n.test.js:307-316` (`I18N_VER == i18n.js?v == app.js?v`) utuh.
- `src/frontend/tests/i18n.test.js:104-112`: array `EXPECTED` 7→8 baris + nama tes "seven"→"eight locales".
- `src/frontend/tests/row-actions.test.js:81`: daftar `LANGS` hardcoded + `"hi"` (diketahui saat suite penuh; di luar handover tapi wajib biar hijau).
...
## 2026-09-15 — Bootstrap installer cold-start + README/Quick-Start entry — PM → fullstack-dev + public-writer — PR #28 TERBUKA

**Asal:** user: "bisa gak buatin script buat otomasi install dependency (termasuk python) ketika jalanin aigate untuk pertama kali running?". Klarifikasi scope dulu (installer = ubah sistem, potentially irreversible): Q1=semua platform, Q2=native PM→fallback uv, Q3=bootstrap cold-start terpisah (run.py utuh), Q4=Python+pip(via run.py)+cek-git (peringatan). Lanjutan: "wiki gak pake multi bahasa" (→ W2.2 DITUTUP) + item #2 "push dan pr" + README/Quick-Start menunjuk bootstrap.
**Kepemilikan (A3):** `scripts/bootstrap.*` = berkas level-repo di luar write-root spesialis → fullstack-dev (sesi `ses_f59fe0659ffePyV2ae0qHEjZOU`). README/Quick-Start/varian = public-writer (sesi `ses_f59e70d52ffeFPbtu0k9OUM0Id`; sempat di-block karena agen belum ke-register → user restart opencode, lalu public-writer spawn sukses). PM nol tulis `run.py`/materi publik.
**Per-berkas (kode baru):**
- `scripts/bootstrap.sh` (BARU, 252 baris, +x): reuse Python≥3.10 → native PM (`pkg`/`apt`/`dnf`/`yum`/`pacman`/`zypper`/`brew`, Linux non-root → langsung uv) → fallback userspace `uv` → cek git (warn) → `exec "$PY" run.py "$@"`. `--check-only` deteksi-saja. `set -euo pipefail`, no-sudo, non-interaktif, idempoten, path-independent (`BASH_SOURCE`→repo root, nol hardkode). Konstanta versi berkomentar rujukan `pyproject.toml:11`.
- `scripts/bootstrap.ps1` (BARU, 243 baris): mirror POSIX utk Windows — `py -3`/`python3`/`python` → `winget install -e --id Python.Python.3.12 --silent` (per-user) → uv userspace (`irm https://astral.sh/uv/install.ps1 | iex`) → `& $cmd @a $RunPy @RestArgs; exit $LASTEXITCODE`. `$ErrorActionPreference='Stop'`, `-CheckOnly`, tanpa eskalasi.
- **`run.py` TIDAK diubah** (sudah punya gate 3.10 `run.py:24-46` + auto-pip `run.py:52-70`).
**Per-berkas (publik, 9):** `README.md` + `documents/readme-variants/README.{id,ru,nl,ja,zh,zh-tw,hi}.md` + `documents/pm/wiki-drafts/Quick-Start.md` — command first-run `python run.py` → `bash scripts/bootstrap.sh` (+ Windows `pwsh scripts/bootstrap.ps1`); "What you need" Python→"Nothing"; "When something goes wrong" disesuaikan; `AIGATE_PORT`/`AIGATE_DEV` tetap jalan (env di-forward). Perintah terminal tak diterjemah; klaim Windows TIDAK di-overclaim.
**Verifikasi PM mandiri (bukan telan receipt):** POSIX — `bash -n`=0, `dash -n`=0, `sh -n`=0; `bash scripts/bootstrap.sh --check-only` nyata di Termux (platform=termux, interpreter ke-deteksi, git present, exit 0, TANPA install/launch; idempoten + path-independent). grep `sudo|runas|password` = HANYA komentar. Publik — grep `python run.py` di 9 target = **0**, `bash scripts/bootstrap.sh` = 9/9, `pwsh scripts/bootstrap.ps1` = 9/9; frasa kunci utuh ("eight languages"/"24 AI coding tools"/`http://localhost:8080`/kredit/`aigate` kecil). `git diff --check`=0; `git diff --stat` kosong (nol tracked berubah sebelum staging); scope = HANYA 2 skrip + 9 dokumen publik (+ report agen). Port 8080 tak disentuh (J6). **LIMITATION JUJUR:** `-CheckOnly` Windows + install native (pkg/brew/apt/winget) + uv download BELUM dieksekusi — `pwsh`/`powershell` tak ada di lingkungan (`command -v` kosong) → butuh reviewer Windows nyata.
**Verifikasi PR mandiri (A12/H5, GET API bukan telan create):** PR **#28** `https://github.com/fadhly-permata/AI-Gate/pull/28` — state=open, draft=False, mergeable=True (clean), base=main, head=feat/bootstrap-installer, labels=[documentation, enhancement], 11 berkas +608/−39. Push token via `GIT_ASKPASS` (nilai tak di-argv/dicetak, helper dihapus; `origin` bersih tanpa token).
